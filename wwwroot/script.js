...
async function handleAuth(e) {
  ...
  const endpoint =
    currentMode === "login" ? "/api/rent/login" : "/api/rent/register";
  const payload =
    currentMode === "login"
      ? { email, password }
      : { email, password, roomType };
  ...
  try {
    ...
    document.getElementById("authMessage").textContent =
      "Cannot connect to API. Is server running on " + API_BASE_URL;
  }
}
...
function showDashboard() {
  ...
  document.getElementById("rentDate").textContent =
    currentUser.dateRentWasPaid || "-";
  document.getElementById("powerPaid").textContent = currentUser.powerPaid;
  document.getElementById("powerBalance").textContent =
    currentUser.powerBalance;
  document.getElementById("powerDate").textContent =
    currentUser.datePowerWasPaid || "-";
  document.getElementById("waterPaid").textContent = currentUser.waterPaid;
  document.getElementById("waterBalance").textContent =
    currentUser.waterBalance;
  document.getElementById("waterDate").textContent =
    currentUser.dateWaterWasPaid || "-";
  loadPricing();
}
...
async function loadPricing() {
  try {
    ...
    const data = await res.json();
    document.getElementById("pricingInfo").innerHTML = Object.entries(data)
      .map(
        ([k, v]) =>
